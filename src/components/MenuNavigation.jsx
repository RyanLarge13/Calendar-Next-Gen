...
const MenuNavigation = () => {
  ...
  return (
    ...
    >
      ...
      <button
        className="bottom-[-15px] absolute right-5 left-5 p-2 rounded-md shadow-md bg-cyan-100 flex justify-center items-center"
        onClick={() => setShowCategory(null)}
      >
        <AiTwotoneHome />
      </button>
      <button
        className="bottom-[-35px] absolute right-10 left-10 md:right-30 md:left-30 lg:right-80 lg:left-80 p-1 rounded-md shadow-md bg-white flex justify-center items-center"
        onClick={() => setHide((prev) => !prev)}
      >
        {hide ? <MdOutlineKeyboardHide /> : <TbKeyboardHide />}
      </button>
    </motion.div>
  );
};
