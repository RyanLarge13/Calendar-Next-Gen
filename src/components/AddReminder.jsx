...
>
  <div className="space-y-5 sm:space-y-6">
    ...
    {quickTimeSelect.fifteen || quickTimeSelect.thirty || quickTimeSelect.hour ? (
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs opacity-70">No schedule changes, just alerts</p>
          </div>
        </div>
    ...
    <div className="space-y-3">
      {[
        ...
        >
          ...
          {value && (
            <p className={`mt-3 inline-flex items-center rounded-xl border px-3 py-1 text-xs font-semibold ${
              preferences.darkMode
    ...
    <div className="space-y-3">
      {todaysEvents.loading ? (
        <p className={`text-sm ${preferences.darkMode ? "text-white/60" : "text-slate-500"}`}>Loading…</p>
      ) : todaysEvents.events.length > 0 ? (
        <>
          <p className={`text-sm font-semibold ${preferences.darkMode ? "text-white/70" : "text-slate-600"}`}>
          ...
          {todaysEvents.events.map((event) => (
            <button
              key={event.id}
              onClick={() =>
                setEventForReminder((prev) => (prev?.id === event.id ? null : event))
              }
              className={`
              ...
              `}
            >
              <div className={`${event.color} absolute left-0 top-0 bottom-0 w-2 rounded-l-2xl`} />
